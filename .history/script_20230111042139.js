...
function removeSlice(index) {
	...
	for (let i = 0; i < slices.length; i++) {
		choiceContainer.innerHTML += `<div class="choice"><button onclick="removeSlice(${i})>Remove</button>${slices[i].sliceName}</div>`;
	}
}
...
function addSlice() {
	...
	drawSlices();
	choiceContainer.innerHTML += `<div class="choice"><button onclick="removeSlice(${
		slices.length - 1
	})">Remove${sliceName}</div>`;
}
