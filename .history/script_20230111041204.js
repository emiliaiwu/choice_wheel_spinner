...
function removeSlice(index) {
	...
	for (let i = 0; i < slices.length; i++) {
		choiceContainer.innerHTML += `<div><div class="choice" onclick="removeSlice(${i})>${slices[i].sliceName}</div></div>`;
	}
}
...
function addSlice() {
	...
	drawSlices();
	choiceContainer.innerHTML += `<div></div><div class="choice" onclick="removeSlice(${
		slices.length - 1
	})">${sliceName}</div>`;
}
