...
function removeSlice(index) {
	...
	for (let i = 0; i < slices.length; i++) {
		choiceContainer.innerHTML += `<div><button onclick="removeSlice(${i})">Remove</button> ${slices[i].sliceName}</div>`;
	}
}

function addSlice() {
	let sliceName = choiceContainer.value;
	slices.push({ sliceName });
	drawSlices();
	choiceContainer.innerHTML += `<div><button onclick="removeSlice(${
		slices.length - 1
